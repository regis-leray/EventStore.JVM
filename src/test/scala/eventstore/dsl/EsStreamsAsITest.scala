...
import eventstore.ProjectionsClient.ProjectionCreationResult.ProjectionAlreadyExist
...
class EsStreamsAsSpec extends TestConnection /*with ScalaFutures
  with Matchers
  with EitherValues
...
//      whenReady(failure){ failure =>
//        failure shouldBe a [StreamNotFoundException]
//      }
  ...
  "Write Event Store" should {
    ...
    "append one element" in new TestScope{
      whenReady(connection.getStreamAs[Book](streamId).append(GodBook(name = "joe")) ){ result =>
        result.right.value shouldBe a [WriteEventsCompleted]
      }
    }
    ...
    "append multiple elements of same type" in new TestScope{
      ...
      whenReady(connection.getStreamAs[GodBook](streamId).appendSeq(books).failOnError){ result =>
    ...
    "append multiple with " in new TestScope{
      ...
      whenReady(connection.getStreamAs[Book](streamId).appendSeq(books).failOnError){ result =>
        result shouldBe Done
      }
    }
  }
